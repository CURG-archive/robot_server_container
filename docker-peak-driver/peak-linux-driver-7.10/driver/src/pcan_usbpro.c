                                ...
                                void *pv, int maxlen)
{
	...
	for (;;)
	{
		...
		if (atomic_read(&usb_if->cmd_sync_complete))
		{
			...
			{
				/* Should read into tmp if record is present. If not, */
				/* loop again... */
				rec_counter = le32_to_cpu(*(uint32_t *)pc);

				/* This rec_counter sometimes contains other info in HIWORD */
				rec_counter &= 0xffff;
...
int pcan_usbpro_get_device_nr(struct pcandev *dev, uint32_t *p_device_nr)
{
   ...
   err =  pcan_usbpro_cmd_send(u->usb_if, &ub);
	if (err == 0)
	{
		...
		if (p_device_nr != NULL) 
			*p_device_nr = le32_to_cpu(pdn->serial_num);
	}
...
/*
 * int pcan_usbpro_set_serial_nr(struct pcan_usb_interface *usb_if, 
 *                               uint32_t dwSNR)
 */
static 
int pcan_usbpro_set_serial_nr(struct pcan_usb_interface *usb_if, uint32_t dwSNR)
{
	int err;
	...
	{
		uint32_t le32_snr = cpu_to_le32(dwSNR);
		err = usb_control_msg(usb_if->usb_dev, 
			                   usb_rcvctrlpipe(usb_if->usb_dev, 0),
			                   USB_VENDOR_REQUEST_FKT,
			                   USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_OTHER,
			                   USB_VENDOR_REQUEST_wVALUE_SETFKT_SERNER,
			                   0, &le32_snr, sizeof(le32_snr),
...
uint32_t pcan_usbpro_handle_response_rtt(struct pcan_usb_interface *usb_if)
{
	...
	if (usb_if->rtt != 0) return usb_if->rtt;
#endif

	delta_us = pcan_timeval_delta_us(&usb_if->tv_request, &usb_if->tv_response);

	if (usb_if->rtt == 0) usb_if->rtt = delta_us;
                                     ...
                                     struct pcan_usbpro_calibration_ts_rx_t *ts)
{
   ...
   dev_frame_nb = (raw_frame_index & PCAN_USBPRO_PRECISION_MASK) << 3 
	...
	/* WARNING: must be done asynchronously (no schedule()!) since we already */
	/* come from a completion handler */
                           ...
                           uint8_t *msg_addr, int msg_len)
{
	                                                                 ...
	                                                                 rec_count--)
	{
		...
		case DATA_TYPE_USB2CAN_STRUCT_CANMSG_RX_0:
		case DATA_TYPE_USB2CAN_STRUCT_CANMSG_RTR_RX:
			err = pcan_usbpro_handle_canmsg_rx(usb_if, &pr->canmsg_rx);
			if (err < 0) goto fail;
			break;
		...
		case DATA_TYPE_USB2CAN_STRUCT_BUSLAST_RX:
			err = pcan_usbpro_handle_buslast_rx(usb_if, &pr->buslast_rx);
			break;
                           ...
                           uint8_t *buffer_addr, int *buffer_size)
{
	...
	while (usb_msg.rec_buffer_len < buffer_high_water)
	{
		...
		if (can_msg.MSGTYPE & MSGTYPE_SINGLESHOT) flags |= 0x08;
		if (can_msg.hw_self_receive) flags |= 0x04;
#endif
		if (can_msg.MSGTYPE & MSGTYPE_EXTENDED) flags |= 0x02;
		if (can_msg.MSGTYPE & MSGTYPE_RTR) flags |= 0x01;
	...
	*buffer_size = (*usb_msg.u.rec_counter == 0) ? 0 : usb_msg.rec_buffer_len;
...
int pcan_usbpro_init(struct pcan_usb_interface *usb_if)
{
	...
	usb_if->device_set_snr = pcan_usbpro_set_serial_nr;
	...
	/* Firmware info */
	{
		...
		if (pcan_usbpro_request(usb_if, 
		                        ...
		                        &fi, sizeof(fi)) >= 0)
		{
			printk(KERN_INFO "%s: PCAN-USB-PRO fw v%d.%d.%d (%02d/%02d/%02d)\n",
			       DEVICE_NAME, 
	...
	/* Bootloader info */
	{
		...
		if (pcan_usbpro_request(usb_if, 
		                        ...
		                        &bi, sizeof(bi)) >= 0)
		{
			printk(KERN_INFO "%s: bootloader v%d.%d.%d (%02d/%02d/%02d)\n",
			       DEVICE_NAME, 
			       bi.version[0], bi.version[1], bi.version[2],
			       bi.day, bi.month, bi.year);

			printk(KERN_INFO "%s: S/N=%08X.%08X hw=0x%08x.0x%08x\n",
			       DEVICE_NAME, 
	...
	/* Device Number */
	{
		...
		if (pcan_usbpro_request(usb_if, 
		                        ...
		                        &dnr, sizeof(dnr)) >= 0)
		{
			printk(KERN_INFO "%s: Device #0x%08x\n",
